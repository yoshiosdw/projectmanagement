...
mock.onPost('/auth/login').reply(request => {
  ...
  if (user) {
    try {
      const sinarjoAccessToken = userTokens[user.id]

      // We are duplicating user here
      const sinarjoUserData = { ...user }

      const userOutData = Object.fromEntries(Object.entries(sinarjoUserData)
        .filter(([key, _]) => !(key === 'password' || key === 'abilities')))

      const response = {
        sinarjoUserAbilities: sinarjoUserData.abilities,
        sinarjoAccessToken,
        sinarjoUserData: userOutData,
      }


      //   const sinarjoAccessToken = jwt.sign({ id: user.id }, jwtSecret)
      return [200, response]
    }
    catch (e) {
      errors = { email: [e] }
...
mock.onPost('/auth/register').reply(request => {
  ...
  if (!errors.username && !errors.email) {
    ...
    console.log('sinarjoUserData :>> ', sinarjoUserData)
    database.push(sinarjoUserData)

    const sinarjoAccessToken = userTokens[sinarjoUserData.id]
    const { password: _, abilities, ...user } = sinarjoUserData

    const response = {
      sinarjoUserData: user,
      sinarjoAccessToken,
      sinarjoUserAbilities: abilities,
    }
