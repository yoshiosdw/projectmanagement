...
const fetchOrganization = async (page, perPage, find) => {
  loading.value = true
  try {
    ...
    totalOrganizations.value = ret.data.meta.total
...
const deleteData = async id => {
  loading.value = true
  try {
    const ret = await axiosIns.delete(`/organizations/${id}` )

    fetchOrganization(page.value, perPage.value, find.value)
  } catch (error) {
    Swal.fire({
      title: 'LBG',
...
const paginationData = computed(()=>{
  ...
  return `Showing ${firstIndex} to ${lastIndex} of ${totalOrganizations.value}`
})
</script>
      ...
      <div>
        <VCardText class="d-flex gap-4">
          ...
          <VTextField
            ...
            @keydown.enter="findOrganization"
          />
          ...
          <VTable>
            ...
            <tbody>
              ...
              >
                ...
                <td>
                  <div class="d-flex flex-column">
                    <p>{{ org.organization_types.name }}</p>
                  </div>
                </td>
                    ...
                    >
                      ...
                      <VMenu activator="parent">
                        <VList>
                          <VListItem
                            value="duplicate"
                            :to="{ name: 'organization-id', params: {id : org.guid}}"
                          >
                            <template #prepend>
                              <VIcon
                                color="warning"
                          ...
                          <VListItem
                            value="duplicate"
                            @click="btnDeleteClickHandler(org.guid)"
                          >
                            <template #prepend>
                              <VIcon
                                color="error"
