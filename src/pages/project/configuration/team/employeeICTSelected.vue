...
const fetchEmployee = async (page, perPage, find) => {
  try {
    ...
    const response = await axiosIns.get(`/employment/ict`, {
...
const postingData = async () => {
  ...
  try {
    // Iterasi melalui setiap item dalam selectedData
    for (const employeeId of selectedData.value) {
      const params = {
        employee_id: employeeId, // Menggunakan employee_id tunggal
        project_teams_id: props.teamId, // Tetap menggunakan project_team_id yang sama
      }

      // Kirim permintaan POST untuk setiap employee_id
      await axiosIns.post('/projectTeam/lines', params)
    }

    // Emit event setelah semua data berhasil diposting
    emits('postSuccess')
    showLoading.value = false
    isVisible.value = false
    selectedData.value = [] // Reset selectedData setelah berhasil
  } catch (error) {
    isVisible.value = true
    showLoading.value = false
    console.log(error)
    ...
    if (error.response && error.response.data && error.response.data.message) {
      errorMessage = error.response.data.message.message || 'Unknown error occurred'
    }
