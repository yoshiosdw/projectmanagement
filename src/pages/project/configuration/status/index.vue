...
const fetchStatus = async (page, perPage, find) => {
...
const deleteData = async id => {
  ...
  try {
    const ret = await axiosIns.delete(`/projectStatus/${id}` )

    fetchStatus(page.value, perPage.value, find.value),
        ...
        <VRow>
          <VCol cols="12">
            <VCardText class="d-flex gap-4">
              ...
              <Add
                @closed="getClosed"
                @saved="fetchStatus(page.value, perPage.value, find.value)"
              />
            </VCardText>
          </VCol>
        </VRow>
