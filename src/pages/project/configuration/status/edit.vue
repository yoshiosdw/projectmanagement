...
// const statusId = computed(() => props.headerId)
const statusId = ref(props.headerId)
...
const fetchStatus = async id => {
  ...
  try {
    const ret = await axiosIns.get(`/projectStatus/${id}` )

    showLoading.value = false

    statusData.value = ret.data.data[0]
    code.value = statusData.value.code
    description.value = statusData.value.description
...
const modifyStatus = async id => {
  showLoading.value = true
  try {
    const ret = await axiosIns.patch(`/projectStatus/${id}`, {
      code            : code.value,
  ...
  <div>
    ...
    >
      ...
      <VCard :loading="showLoading">
        <VCardTitle>Project Status</VCardTitle>
        <VDivider />
