...
const postMaintenance = async maintainById => {
  loading.value = true
  try {

    const payload = {
      cost: cost.value,
      complete_date: formatDateTimeMySql(completeDate.value),
    }

    const ret = await axiosIns.post(`/maintenances/done/${maintainById}`, payload )
...
const onSubmit = () => {
  refMaintenanceForm.value?.validate().then(({ valid: isValid }) => {
    if(isValid) postMaintenance(maintainById)
  })
}
</script>
            ...
            <VRow>
              ...
              <VCol cols="6">
                <VDateInput  
                  ...
                  density="compact"
                  :rules="[requiredValidator]"
                  :config="{miDate:completeDate, enableTime: true, dateFormat: 'Y-m-d H:i', time_24hr: true }"
                />
              </VCol>
            </VRow>
          </VCardText>
