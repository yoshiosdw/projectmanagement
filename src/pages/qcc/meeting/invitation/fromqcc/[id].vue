...
const qccDataId = ref(route.params.id)
const qccData = ref([])
...
const fetchQcc = async qccDataId => {
  try {
    const ret = await axiosIns.get(`/qcc/qccs/${qccDataId}` )

    qccData.value = ret.data.data[0]

    circleId.value =  qccData.value.circle_id
    circleCode.value =  qccData.value.circle?.code
    circle.value =  qccData.value.circle?.name
  } catch(error) {
